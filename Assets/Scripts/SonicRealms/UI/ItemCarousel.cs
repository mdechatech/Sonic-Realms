...
namespace SonicRealms.UI
{
    ...
    public class ItemCarousel : MonoBehaviour
    {
        ...
        private ItemCarouselArranger _arranger;

        [SerializeField, Space, Foldout("Items")]
        ...
        {
            _arranger = GetComponentInChildren<ItemCarouselArranger>();
        ...
        private IEnumerator Coroutine_CheckDeselection()
        {
            ...
            if (FocusMode == FocusBehavior.MustBeSelected || FocusMode == FocusBehavior.MustBeSubmitted)
            {
                if (FocusMode == FocusBehavior.MustBeSelected)
                {
                    ...
                    yield return new WaitForUnityEvent<BaseEventData>(_deselected);
                }
                else if (FocusMode == FocusBehavior.MustBeSubmitted)
                {
                    ...
                    yield return new WaitForAny(true,
                        new WaitForUnityEvent<BaseEventData>(_canceled),
                        new WaitForUnityEvent<BaseEventData>(_deselected));
                }
